...
class FWIOT_device(models.Model):
    ...
    alerts = fields.One2many('fwiot_device_alert',
                             'device_id', string='Alert trigger')
    ...
    def _compute_device_status(self):
        for each in self:
            if each.state != 'confirm' or not each.last_online:
                ...
                dt = (datetime.now() -
                      each.last_online).total_seconds() / (60 * 60 * 24)
                if dt < 1:
    ...
    def action_setting(self):
        ...
        last = self.env[self._get_device_implement().get('setting', {}).get(
            'model')].search([], limit=1, order='id desc')
    ...
    def get_schedule(self):
        sch_model = self._get_device_implement().get(
            'data', {}).get('schedule_id', False)
        if not sch_model:
            return
    ...
    def action_schedule(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            'fw_iot.fwiot_device_cron_wizard_action')
        action['context'] = dict(self.env.context)
    ...
    def get_condition_fields(self, device_id):
        ...
        for each in dv.get('alert', {}).get('fields', []):
            ...
            ff.append(
                (each, langdb._get_ids('ir.model.fields,field_description',
                 'model', self.env.user.lang, [f.id]).get(f.id) or each)
            )
        return ff
    ...
    def action_view_history(self):
        ...
        if not self.is_implement:
            return
        action = self.env["ir.actions.actions"]._for_xml_id(
            'fw_iot.fwiot_device_status_action')
        action['context'] = {}
        action['domain'] = [('device_id', '=', self.id)]
        return action
